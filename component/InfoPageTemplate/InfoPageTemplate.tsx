...
const HeroSection = memo(({ hero }: { hero: PageData['hero'] }) => (
...
const IntroSection = memo(({ intro }: { intro: PageData['intro'] }) => (
  <section className="grid md:grid-cols-2 gap-12 items-center">
    <div>
      <h2 className="text-3xl font-bold text-slate-900 mb-4">{intro.title}</h2>
      <p className="text-slate-600 leading-relaxed mb-6">{intro.content}</p>
...
const ServiceCard = memo(({ service }: { service: PageData['services'][0] }) => (
  <div className="bg-white p-6 rounded-xl border border-slate-100 hover:border-teal-200 hover:shadow-md transition-all group">
    ...
    <p className="text-slate-500 text-sm mb-4 min-h-[60px]">{service.desc}</p>
    <button className="text-teal-600 text-sm font-semibold flex items-center hover:underline">
      {service.action} <ArrowRight size={14} className="ml-1" />
    </button>
  </div>
));
ServiceCard.displayName = 'ServiceCard';

const ServicesSection = memo(({ services }: { services: PageData['services'] }) => (
  <section>
...
const CTASection = memo(({ cta }: { cta: PageData['cta'] }) => (
  <section className="bg-slate-900 rounded-3xl p-8 md:p-16 text-white grid md:grid-cols-2 gap-12 relative overflow-hidden">
    ...
    <div className="relative z-10">
      ...
      <p className="text-slate-300 text-lg mb-8">{cta.description}</p>
...
const TableSection = memo(({ table }: { table: PageData['table'] }) => (
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-left text-sm text-slate-600">
        <thead className="bg-slate-50 text-slate-900 font-semibold uppercase text-xs">
          <tr>
            {table.headers.map((h, i) => <th key={i} className="p-6">{h}</th>)}
          </tr>
        </thead>
...
const FAQItem = memo(({ faq }: { faq: PageData['faqs'][0] }) => (
...
const FAQSection = memo(({ faqs }: { faqs: PageData['faqs'] }) => (
...
export default memo(InfoPageTemplate);
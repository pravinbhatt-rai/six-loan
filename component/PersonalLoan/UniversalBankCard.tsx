  ...
  useEffect(() => {
    const fetchLoans = async () => {
      try {
        ...
        // Try multiple fetch approaches to handle CORS issues
        const fetchPromises = [
          ...
          // Approach 3: With different headers
          fetch(apiUrl, {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
        ...
        for (let i = 0; i < fetchPromises.length; i++) {
          try {
            ...
            console.log(`Approach ${i + 1} status:`, response.status, response.statusText);
            
            if (response.ok || (i === 1 && response.type === 'opaque')) { // no-cors mode returns opaque responses
              if (response.ok) {
                ...
                console.log("No-CORS request sent (can't read response)");
              }
            }
          } catch (innerError) {
            console.log(`Approach ${i + 1} failed:`, innerError);
          }
        }
        ...
        if (!successfulResponse) {
          ...
          try {
            ...
            console.log("Direct fetch test - Response text:", text.substring(0, 200));
            
            setError(`Server error: ${testResponse.status} ${testResponse.statusText}`);
          } catch (finalError: any) {
            console.error("Final fetch error:", finalError);
            setError(`Network error: ${finalError.message}. Please check if the backend server is running on port 4000.`);
          }
        }
    ...
    return (
      <div className="w-full max-w-6xl mx-auto p-4">
        <h1 className="text-3xl font-bold text-gray-900 mb-6 font-serif">
          {title || `Best ${categorySlug.replace(/-/g, ' ')} Offers`}
        </h1>
        <div className="text-center py-10">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-teal-500 mx-auto mb-4"></div>
          <p className="text-gray-600">Loading loan offers...</p>
          <p className="text-sm text-gray-400 mt-2">
            Connecting to: {process.env.NEXT_PUBLIC_BACKEND_URL}/api/loans/category/{categorySlug}
          </p>
        </div>
      </div>
    );
      ...
      <div className="w-full max-w-6xl mx-auto p-4">
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-6">
          ...
          <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded">
            <h4 className="font-bold text-blue-800 mb-2">Quick Fix Instructions:</h4>
            
            <div className="space-y-3">
              <div>
                <h5 className="font-semibold text-blue-700 mb-1">1. Check if Express server is running:</h5>
                <div className="bg-gray-100 p-3 rounded text-sm">
                  <p>Open terminal and run:</p>
                  <code className="block bg-black text-green-400 p-2 rounded mt-1">
                    curl http://localhost:4000/api/loans/category/personal-loan
                  </code>
                  <p className="mt-2">Expected response: JSON data with loan information</p>
                </div>
              </div>
              
              <div>
                <h5 className="font-semibold text-blue-700 mb-1">2. Add CORS to Express server:</h5>
                <div className="bg-gray-100 p-3 rounded text-sm">
                  <p>In your Express server file (server.ts/app.ts), add:</p>
...
// npm install cors`}
              ...
              <div>
                <h5 className="font-semibold text-blue-700 mb-1">3. Test API directly:</h5>
                <a 
                  href={`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/loans/category/${categorySlug}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-block bg-teal-500 text-white px-4 py-2 rounded hover:bg-teal-600 transition-colors"
                >
                  Click here to test API in browser
                </a>
                <p className="text-sm text-gray-600 mt-1">
                  If this link works but the component doesn't, it's a CORS issue.
                </p>
              </div>
              
              <div>
                <h5 className="font-semibold text-blue-700 mb-1">4. Check browser console:</h5>
                <p className="text-sm">Press F12 → Console tab → Look for CORS errors</p>
              </div>
            </div>
          </div>
          ...
          >
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-3 mb-6">
                <div className="w-10 h-10 overflow-hidden rounded-full bg-gray-50 flex items-center justify-center border border-gray-200">
                  {card.bankLogoUrl ? (
                    <img 
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                ...
                <div>
                  ...
                  {card.processTimeValue && (
                    <p className="text-xs text-gray-500 mt-1">{card.processTimeValue}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {card.approvalScore && card.approvalScore > 0 && (
                    <div className="flex items-center gap-1 mt-1">
                      <div className="w-full bg-gray-200 rounded-full h-1.5">
                        <div 
                          className="bg-teal-500 h-1.5 rounded-full" 
                          style={{ width: `${Math.min(card.approvalScore, 100)}%` }}
                        ></div>
                      </div>
                      <span className="text-xs text-gray-500">{card.approvalScore}%</span>
                    </div>
                  )}
                </div>
